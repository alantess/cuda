...
/*   m: number of elemnts in the mask */
__global__ void convolutional_1d(int *array, int *mask, int *result, int n,
                               int m) {
...
void verify(int *array, int *mask, int *result, int n , int m){
  int radius = m / 2;
  int temp;
  int start;
  for (int i = 0; i < n; i++) {
...
int main(){
  ...
  // Call the kernel
  convolutional_1d<<<GRID, THREADS>>>(d_array, d_mask, d_result, n, m);

  // Copy back the result
  cudaMemcpy(h_result.data(), d_result, bytes_n, cudaMemcpyDeviceToHost);

  // Verify the result
  verify(h_array.data(), h_mask.data(), h_result.data(), n, m);
